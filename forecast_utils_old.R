...
get.synthetic.epi.param <- function(source){
	...
	prm.name.raw <- get.prm.names.from.source(source.string = source)
	...
	GImean.true   <- get.prm.value.from.source(inc.tb$source, prm.name = "GImean")[1]
	GIvar.true    <- get.prm.value.from.source(inc.tb$source, prm.name = "GIvar")[1]
	
	return(c(nE.true = nE.true,
			 nI.true = nI.true,
			 DOL.true = DOL.true,
			 DOI.true = DOI.true,
			 GImean.true = GImean.true,
			 GIvar.true = GIvar.true))
}