...
####  === W A R N I N G ===
####
#### THIS SHOULD BE 'EpiEstim' R PACKAGE
#### BUT EDITED BY DAVID CHAMPREDON
#### --> LOOK FOR 'DC_EDIT' IN THIS CODE
####
...
OverallInfectivity <-function (I,SI.Distr)
{
	...
	if(abs(sum(SI.Distr)-1)>0.01)
	{
		...
		# DC_EDIT
		warning("SI.Distr renormalized to 1: the support for SI.Distr is larger than data length")
		SI.Distr <- SI.Distr/sum(SI.Distr)
	}
	lambda <- vector() 
